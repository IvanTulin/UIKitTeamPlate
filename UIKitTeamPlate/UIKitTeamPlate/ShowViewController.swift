...
final class ShowViewController: UIViewController {
    ...
    private let labelOfTheSecondName: UILabel = {
    ...
    private let labelSecondWord: UILabel = {
    ...
    private func configure() {
        ...
        view.addSubview(labelFirstWord)
        view.addSubview(labelOfTheSecondName)
        view.addSubview(labelSecondWord)
        view.addSubview(startButton)
        
        startButton.addTarget(self,
                              action: #selector(getItInReverseOrder),
                              for: .touchUpInside)
    }
    
    @objc private func getItInReverseOrder() {
        if let text = labelFirstWord.text {
            labelSecondWord.text = model.flipOver(text)
        }
    }
}
